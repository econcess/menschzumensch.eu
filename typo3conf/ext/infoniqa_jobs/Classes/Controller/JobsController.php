...
//+3078/econcess
use CodingMs\InfoniqaJobs\Service\XmlService;
...
//-1066/econcess
    ...
    {
        // Fetch jobs XML
        $this->xmlService = GeneralUtility::makeInstance(XmlService::class);
...
//+3111/econcess
			if(isset($offer['jobGroup']['values']['value']) && is_string($offer['jobGroup']['values']['value'])) {
				$this->jobgroups[$offer['jobGroup']['values']['value']] = $offer['jobGroup']['values']['value'];	
			}
			if(isset($offer['locations']['location']['values']['value']) && is_string($offer['locations']['location']['values']['value'])) {				
...
//-483/econcess
            //
            if(isset($offer['notation']['values']['value']) && is_string($offer['notation']['values']['value'])) {
            ...
            //
            if(isset($offer['additionalBenefit']['values']['value']) && is_string($offer['additionalBenefit']['values']['value'])) {
...
//+1702/econcess
				...
				return $b['demandDate'] <=> $a['demandDate'];
//-1702/econcess
			});
		}
//+679/econcess
...
//+1871/econcess
					...
					$metaTagManager->addProperty('og:title', ($jobOffer['description']['values']['value']).' | Infoniqa');
					...
					$metaTagManager->addProperty('twitter:title', ($jobOffer['description']['values']['value']).' | Infoniqa');
//-1871/econcess
//-856/econcess
                }
            }
//+2339/econcess
			if (!count($jobOffer)) {
				$redirectUid = 1; 
				if($this->settings['joblist']>0) $redirectUid = $this->settings['joblist'];
				$uriBuilder = $this->uriBuilder;
				$uri = $uriBuilder
					->setTargetPageUid($redirectUid)
					->build();
